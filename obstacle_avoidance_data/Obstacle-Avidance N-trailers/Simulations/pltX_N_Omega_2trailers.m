...
for j=1:length(S.stability.theta1Range)*length(S.stability.theta2Range)
    iInindx     = [];
    iOutindx    = [];
    trajIn      = [];
    trajOut     = [];
    for i=j:length(S.stability.theta1Range)*length(S.stability.theta2Range):S.config.stab.nx0Stab    
    ...
    if ~isempty(trajIn)
        ...
        figure; hold on; grid on; xlabel('x');ylabel('y');zlabel('theta'); title(['Nc= ',num2str(S.config.Nc),', Vol= ',num2str(X_N_Omega_Iprojection.volume),...
            ',','$\beta_1=$',num2str(S.stability.theta1Range(ceil(j/length(S.stability.theta1Range)))),',','$\beta_2=$',num2str(S.stability.theta2Range(1+mod(j-1,length(S.stability.theta2Range))))])        
        if plt3d
        ...
        % xlim([-3 3]);ylim([-3 3]);zlim([-pi pi]); daspect([1 1 1])
    end
end
