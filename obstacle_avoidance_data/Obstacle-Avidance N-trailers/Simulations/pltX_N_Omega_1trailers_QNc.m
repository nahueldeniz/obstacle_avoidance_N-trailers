...
numPoints = size(S.trajStab,3);
trajStab = S.trajStab;
Vtx = [];

Vtx = [Vtx; [min(trajStab(coordX0,1,:)),min(trajStab(coordY0,1,:)),min(trajStab(coordTheta0,1,:))]];
Vtx = [Vtx; [min(trajStab(coordX0,1,:)),min(trajStab(coordY0,1,:)),max(trajStab(coordTheta0,1,:))]];
Vtx = [Vtx; [min(trajStab(coordX0,1,:)),max(trajStab(coordY0,1,:)),min(trajStab(coordTheta0,1,:))]];
Vtx = [Vtx; [min(trajStab(coordX0,1,:)),max(trajStab(coordY0,1,:)),max(trajStab(coordTheta0,1,:))]];
Vtx = [Vtx; [max(trajStab(coordX0,1,:)),min(trajStab(coordY0,1,:)),min(trajStab(coordTheta0,1,:))]];
Vtx = [Vtx; [max(trajStab(coordX0,1,:)),min(trajStab(coordY0,1,:)),max(trajStab(coordTheta0,1,:))]];
Vtx = [Vtx; [max(trajStab(coordX0,1,:)),max(trajStab(coordY0,1,:)),min(trajStab(coordTheta0,1,:))]];
Vtx = [Vtx; [max(trajStab(coordX0,1,:)),max(trajStab(coordY0,1,:)),max(trajStab(coordTheta0,1,:))]];

VtxCvcH                 = convhull(Vtx);
X_N_Omega_Iprojection   = Polyhedron(Vtx);
X_N_Omega_Iprojection   = X_N_Omega_Iprojection.minVRep;

% figure; hold on; grid on; xlabel('x');ylabel('y');%zlabel('theta'); title(['Nc= ',num2str(S.config.Nc),', Vol= ',num2str(X_N_Omega_Iprojection.volume)])
% xlim([-3 3]); ylim([-3 3]); 
...
X1 = NaN(1,length(trajStab));
Y1 = NaN(1,length(trajStab));
Z1 = NaN(1,length(trajStab));

Xtraj = NaN(1,length(trajStab),S.config.Nc);
Ytraj = NaN(1,length(trajStab),S.config.Nc);
Ztraj = NaN(1,length(trajStab),S.config.Nc);

for i=1:length(trajStab)
    X1(i) = trajStab(coordX0,1,i);
    Y1(i) = trajStab(coordY0,1,i);
    Z1(i) = trajStab(coordTheta0,1,i);
    %
    Xtraj(1,(i-1)*S.config.Nc+1:i*S.config.Nc) = trajStab(coordX0,:,i);
    Ytraj(1,(i-1)*S.config.Nc+1:i*S.config.Nc) = trajStab(coordY0,:,i);
    Ztraj(1,(i-1)*S.config.Nc+1:i*S.config.Nc) = trajStab(coordTheta0,:,i);
end
...
% for j=1:10
%     for i=j:10:numPoints
%          if plt3d            
%     %                 sphHandle = plotSpheres(S.trajStab{i}(coordX0,1),S.trajStab{i}(coordY0,1),S.trajStab{i}(coordTheta0,1),1.0,'b');
%                 plot3(trajStab(coordX0,1,i),trajStab(coordY0,1,i),trajStab(coordTheta0,1,i),'bo','markersize',30);
%     %             plot3(trajStab(coordX0,end,i),trajStab(coordY0,end,i),trajStab(coordTheta0,end,i),'ro','markersize',3);
%     %                 alpha(sphHandle, 0.25);
%     %             plot3(trajStab(coordX0,:,i),trajStab(coordY0,:,i),trajStab(coordTheta0,:,i),'c')
%     %             plot(trajStab([coordX0,coordY0,coordTheta0],:,i)','c')
%     %                 sphInHandle = plotSpheres(S.trajStab{i}(coordX0,end),S.trajStab{i}(coordY0,end),S.trajStab{i}(coordTheta0,end),0.65,'r');
%     %                 alpha(sphInHandle, 0.35);
%          end
%     end
% end
...
% for j=1:length(S.stability.theta1Range)
%     iInindx     = [];
%     iOutindx    = [];
%     trajIn      = [];
%     trajOut     = [];
...
%         X_N_Omega_Iprojection   = X_N_Omega_Iprojection.minVRep;
%         %
%         figure; hold on; grid on; xlabel('x');ylabel('y');zlabel('theta'); title(['Nc= ',num2str(S.config.Nc),', Vol= ',num2str(X_N_Omega_Iprojection.volume),',','$\beta_1=$',num2str(S.stability.theta1Range(j))])        
%         if plt3d
%             xnHandle    = X_N_Omega_Iprojection.plot('color','b');
%             alpha(xnHandle,0.15);
%             xxhandle    = XXplt.plot('color','r');
...
%             for i=iInindx
% %                 sphHandle = plotSpheres(S.trajStab{i}(coordX0,1),S.trajStab{i}(coordY0,1),S.trajStab{i}(coordTheta0,1),1.0,'b');
% sphHandle = plot3(S.trajStab{i}(coordX0,1),S.trajStab{i}(coordY0,1),S.trajStab{i}(coordTheta0,1),'bo','markersize',3);
% %                 alpha(sphHandle, 0.25);
% %                 plot3(S.trajStab{i}(coordX0,:),S.trajStab{i}(coordY0,:),S.trajStab{i}(coordTheta0,:),'c')
% %                 sphInHandle = plotSpheres(S.trajStab{i}(coordX0,end),S.trajStab{i}(coordY0,end),S.trajStab{i}(coordTheta0,end),0.65,'r');
% %                 alpha(sphInHandle, 0.35);
%             end
%         else
